...
class ParticipantBot(ABC):
    ...
    def next_message(
        ...
        Args:
            context_so_far: Preceding messages in the thread. ReplayBot ignores
                this; LLMParticipantBot uses it to generate contextually coherent
                responses.
        """
...
class ReplayBot(ParticipantBot):
    ...
    def next_message(
        self,
        context_so_far: list[ConversationRecord] | None = None,  # noqa: ARG002
    ) -> ConversationRecord | None:
...
class SimulatedDataset:
    ...
    def create_with_llm_bots(
        ...
        """Create a HuggingFace IterableDataset backed by LLM-powered participant bots.

        Each thread's turn schedule (author ordering + message count) is taken from
        the historical data, but every message is freshly generated by the appropriate
        LLMParticipantBot and scored by the OracleEvaluator.

        Args:
            raw_data_dir: Directory containing raw batch_conversations*.jsonl files
                (used as turn-schedule templates).
            bot_profiles_dir: Directory containing {author}.json BotProfile files
                produced by the build_profiles CLI.
            participant_backend: LLMBackend used by the participant bots to generate
                utterances. Create with ``make_backend(name, model)``.
            oracle_backend: LLMBackend used by the OracleEvaluator to score synthetic
                messages. May be the same object as participant_backend.
            min_context_turns: Number of preceding turns required before emitting a
                training sample.
            seed: Random seed for per-epoch thread-order shuffling.
        ...
        return IterableDataset.from_generator(
            ...
            gen_kwargs={
                "raw_data_dir": str(raw_data_dir),
                "bot_profiles_dir": str(bot_profiles_dir),
    ...
    def _generate_llm(
        ...
        """Infinite generator for LLM bot mode: yields GRPORecord dicts.

        Loads bot profiles once at startup, then loops over thread templates
        indefinitely, shuffling the order each epoch.
        """
        from grpo_pipeline.bot_profiles import load_all_profiles  # noqa: PLC0415
        from grpo_pipeline.llm_bots import (  # noqa: PLC0415
            LLMConversationEnvironment,
            LLMParticipantBot,
            OracleEvaluator,
        )

        profiles = load_all_profiles(Path(bot_profiles_dir))
        if not profiles:
            raise ValueError(
                f"No bot profiles found in {bot_profiles_dir!r}. "
                "Run: python -m grpo_pipeline.build_profiles --input <raw-data> --output <bot-profiles>"
            )

        oracle = OracleEvaluator(oracle_backend)
        threads = load_all_threads(raw_data_dir)
        if not threads:
            raise ValueError(
                f"No conversation files found in {raw_data_dir!r}. "
        ...
        while True:
            ...
            rng.shuffle(epoch_threads)
            for thread_id, source_file, messages in epoch_threads:
                # Build bots for authors present in this thread
                thread_authors = {m.author for m in messages}
                bots: dict = {}
                for author in thread_authors:
                    profile = profiles.get(author)
                    if profile is None:
                        continue
                    bots[author] = LLMParticipantBot(
                        profile=profile,
                ...
                env = LLMConversationEnvironment(
                    thread_id=thread_id,
                    turn_schedule=messages,
                    bots=bots,
                    oracle=oracle,
                    source_file=source_file,
                )
                for record in env.run_to_records(min_context_turns=min_context_turns):
                    yield record.model_dump()
